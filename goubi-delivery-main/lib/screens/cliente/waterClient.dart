...
class WaterClientScreen extends StatefulWidget {
  const WaterClientScreen({Key? key}) : super(key: key);

  @override
  State<WaterClientScreen> createState() => _HomeWidgetState();
}
...
class _HomeWidgetState extends State<WaterClientScreen> {
  final fb = FirebaseDatabase.instance;
  var isSwitched = true;

  Future<void> setupToken() async {
    await FirebaseMessaging.instance.subscribeToTopic('waterRequest');
  }
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> acceptOrder(WaterRequest waterRequest, String? key) async {
      showDialog(
        context: context,
      ...
      final clientui = waterRequest.uid;
      ...
      refDelivery.update({"orderid": key, "type": "water"}).then((_) {
        refClient.update(
            {"status": "accepted", "orderid": key, "type": "water"}).then((_) {
    ...
    Material _solicitudButton(WaterRequest waterRequest, String? key) {
      return Material(
        elevation: 5,
        ...
        child: MaterialButton(
          ...
          onPressed: () async {
            await acceptOrder(waterRequest, key);
          },
          child: const Text(
            "Aceptar Solicitud",
    ...
    Card _waterCard(WaterRequest waterequest, String? key) {
      return Card(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: _solicitudButton(waterequest, key),
            )
          ],
        ),
      );
                            ...
                            return FirebaseAnimatedList(
                                ...
                                itemBuilder:
                                  ...
                                  WaterRequest waterRequest =
                                      WaterRequest.fromJson(snapshot.value);
